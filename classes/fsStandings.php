...
//-----------------BUILD FOR LARGE AND MEDIUM SCREENS		
		...
		foreach($totals as $uid=>$total){
			...
			$display.= "<div class='cell large-auto medium-auto standingsscores'>".number_format($total)."</div>";
...
//-----------------BUILD FOR SMALL SCREENS
		...
		$display.= "<div class='grid-container full show-for-small-only sm-standingstable'>";
		
		foreach($totals as $uid=>$total){
			//build row with username and total event score
			$display.= "<div class='grid-x sm-standingsrow sm-eventrowu".$uid."'>
							<div class='cell small-8 sm-standingsuser'>".strtoupper($users[$uid]['team'])."</div>
...
//-----------------END BUILD FOR SMALL SCREENS
	...
	private function displayRunningEventStandings($event_id,$surfers,$users,$order,$count,$sorted){
		...
		foreach($order as $uid=>$useless){
			...
			$display.= "<div class='cell small-2 livescores'>
		...
		//-----------------BUILD FOR SMALL SCREENS
				...
				foreach($order as $uid=>$useless){
					//build row with username and total event score
					$display.= "<div class='grid-x sm-standingsrow sm-eventrowu".$uid."'>
									<div class='cell small-7 sm-standingsuser'>".strtoupper($users[$uid]['team'])."</div>
									<div class='cell small-5 sm-standings-livescores'>
										<div class='grid-x'>
											<div class='cell small-6 sm-livetotal'>".number_format($count[$uid]['points'])."</div>
											<div class='cell small-6'>
												<div class='sm-bestscore'>".number_format($count[$uid]['best'])."</div>
												<div class='sm-worstscore'>".number_format($count[$uid]['worst'])."</div>
											</div>
										</div>
									</div>
								</div>";
...
//----------------------START LARGE AND MEDIUM LEADERBOARD
		...
		$display.= "<div class='grid-container full sm-leaguetable show-for-small-only'>";
	...
	private function displayRunningLeagueStandings($event_id,$surfers,$users,$standings,$picks,$count){
		...
		//add up best, worst and current score to users current total leaderboard score
		foreach($count as $uid=>$v){
			$count[$uid]['totalbest'] = $standings[$lastevent][$uid]['pts'] + $count[$uid]['best'];
			$count[$uid]['totalworst'] = $standings[$lastevent][$uid]['pts'] + $count[$uid]['worst'];
			$count[$uid]['totalcurrent'] = $standings[$lastevent][$uid]['pts'] + $count[$uid]['points'];
		}
		
		//create arrays for each for sorting
		foreach($count as $uid=>$v1){
			$bybest[$uid] = $count[$uid]['totalbest'];
			$byworst[$uid] = $count[$uid]['totalworst'];
			$bycurrent[$uid] = $count[$uid]['totalcurrent'];
 		}
		
		//sort arrays highest to lowest
		arsort($bybest);arsort($byworst);arsort($bycurrent);
		...
		//----------------------START LARGE AND MEDIUM LEADERBOARD
											...
											<div class='cell medium-8 medium-8 leaderboard-title-results'>
												...
												for($e=1;$e<=11;$e++){//build title cell for each event
													...
													if($e<=$lastevent){
														...
														//non-empty result
														if($e==$lastevent){	//current event loads up expanded
															$display.="<div class='leaderboard-title title-expanded' id='title".$e."'>".$e."</div>";
														}else{//not current event loads up collapsed
															$display.="<div class='leaderboard-title' id='title".$e."'>".$e."</div>";
														}
				...
				$display.= "							
					...
					foreach($standings[$lastevent] as $uid=>$v1){ //goes through leaguestandings for this event to start with highest scoring user
				...
				foreach($standings[$lastevent] as $uid=>$v1){ 
			
					$display.= "<div class='grid-x sm-lb-row' id='sm-lb-u".$uid."'>
												<div class='cell small-7 sm-lb-username' style='padding-left:10px'>
																				<div class='lb-user-team-sm'>".strtoupper($users[$uid]['team'])."</div>
												</div>
												<div class='cell small-4 sm-lb-total'>".number_format($v1['pts'])."</div>
												<div class='cell small-1 sm-lb-expanduser'> 
...
//----------GET LIVE LEADERBOARD DATA	
			...
			//produce league leaderboard
			$display .= $this->displayRunningLeagueStandings($event_id,$surfers,$users,$standings,$sortedpicks['desc'],$count);
